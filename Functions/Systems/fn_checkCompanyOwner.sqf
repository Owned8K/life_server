...
private _query = format ["SELECT id, name, owner_name, owner_uid, bank FROM companies WHERE owner_uid='%1' LIMIT 1", _playerUID];
...
diag_log format ["Sending result to owner ID: %1", _ownerID];
[_queryResult, _player] remoteExecCall ["life_fnc_companyOwnershipReceived", _ownerID];
